...
async fn test_consolidate_then_deploy() {
    ...
    println!(
        "[K-RWA] Issuer has {} UTXOs before consolidation",
        utxos.len()
    );
    ...
    if utxos.len() > 5 {
        let batches = client
            .consolidate_utxos(&addr, wallet.keypair())
            .await
            .unwrap();
    ...
    let htlc_tx = client
        .deploy_contract(&addr, &htlc, 1_000_000, wallet.keypair())
        .await
        .unwrap();
    println!(
        "[K-RWA] HTLC DEPLOYED! TX: {} P2SH: {}",
        htlc_tx, htlc.p2sh_address
    );
    ...
    let div_tx = client
        .deploy_contract(&addr, &div, 1_001_000, wallet.keypair())
        .await
        .unwrap();
    println!(
        "[K-RWA] Dividend DEPLOYED! TX: {} P2SH: {}",
        div_tx, div.p2sh_address
    );
