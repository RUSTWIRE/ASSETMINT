...
async fn test_deploy_then_spend_clawback() {
    ...
    let _fund = client.send_kas(&bob.address_string(), &fresh_addr, 300_000_000, bob.keypair(), None).await.unwrap();
    ...
    let deploy_tx = client.deploy_contract(&fresh_addr, &contract, 100_000_000, fresh.keypair()).await.unwrap();
    ...
    println!("[K-RWA] Invoking ownerSpend → recipient {}", recipient.address_string());

    match client.spend_contract(&contract, witness, &recipient.address_string(), 50_000_000, 1, fresh.keypair()).await {
        Ok(tx_id) => {
            println!("[K-RWA] ========================================");
