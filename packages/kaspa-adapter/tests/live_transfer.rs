...
async fn test_live_alice_to_bob_transfer() {
    ...
    let balance = client
        .get_balance(&alice_addr)
        .await
        .expect("Failed to get balance");
    println!(
        "[K-RWA] Alice balance: {} sompis ({:.4} KAS)",
        balance,
        balance as f64 / 1e8
    );
    assert!(
        balance > SEND_AMOUNT + 100_000,
        "Alice needs at least 0.101 KAS"
    );
    ...
    let new_balance = client
        ...
        .expect("Failed to get balance");
    println!(
        "[K-RWA] Alice new balance: {} sompis ({:.4} KAS)",
        new_balance,
    ...
    println!(
        "[K-RWA] Balance delta: {} sompis",
        balance as i64 - new_balance as i64
    );
...
async fn test_live_bob_to_alice_return() {
    ...
    const ALICE_ADDRESS: &str =
        "kaspatest:qrl9q9vapkepmgs375v2v329mlc24c0g8zujt52etwm7ragjw4nay2rs8kxh8";
    ...
    let balance = client
        .get_balance(&bob_addr)
        .await
        .expect("Failed to get balance");
    println!(
        "[K-RWA] Bob balance: {} sompis ({:.4} KAS)",
        balance,
        balance as f64 / 1e8
    );
